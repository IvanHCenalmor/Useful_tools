...
def image_converter_from_folder(folder, desired_extension, input_extension):
    for filename in [e for e in os.listdir(folder) if e.endswith(input_extension)]:
...
layout = [[sg.Text('Your File'), sg.InputText('Default Filename', key='-Filename-'), sg.FileBrowse()],
          ...
          [sg.Text('Is a File'), sg.Checkbox('File', 1, key='-isFile-'), sg.Text('Given extension'), sg.OptionMenu(extension_choices, s=(15,2), key='-Input_extension-')],             
    ...
    if event == sg.WIN_CLOSED or event=="Cancel":
        ...
        if values['-isFile-']:
            ...
            if os.path.exists(values['-Folder-']):
                image_converter_from_folder(values['-Folder-'], values['-Extension-'], values['-Input_extension-'])
